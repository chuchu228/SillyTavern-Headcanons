...
const HEADCANON_THEMES = [
    ...
    "Сразу после секса","Игрушки и девайсов","Интим вне кровати","Абсолютное табу в сексе","Голос и взгляд при страсти"
];

// Безопасный запуск расширения
function initHeadcanons() {
    if (document.getElementById('srt-btn')) return;
    
    injectSendButton();
    createPopup();
    console.log('[SRT] Headcanons extension successfully initialized.');
}

// Привязываемся к JQuery SillyTavern без импорта модулей
jQuery(() => {
    // Если у Таверны есть встроенный метод регистрации, используем его глобально
    if (window.SillyTavern?.registerExtension) {
        window.SillyTavern.registerExtension({
            name: MODULE_KEY,
            render: initHeadcanons,
            settings: initHeadcanons
        });
    } else {
        // Если глобальный объект недоступен сразу, запускаем напрямую через DOM
        initHeadcanons();
    }
});
...
function createPopup() {
    ...
    popup.innerHTML = `
      ...
      <div style="padding:10px;display:flex;gap:5px;border-bottom:1px solid rgba(255,255,255,0.05)">
        <button id="srt-scan" class="menu_button" style="flex:1; background:#4a4a4a; color:white; border:none; padding:8px; border-radius:4px; cursor:pointer;">🎲 Сгенерировать темы</button>
      </div>
    ...
    const closeAction = (e) => { e.preventDefault(); popup.style.display = 'none'; };
    document.getElementById('srt-close').addEventListener('click', closeAction);
    document.getElementById('srt-close').addEventListener('touchend', closeAction);
    document.getElementById('srt-scan').addEventListener('click', (e) => { e.preventDefault(); generateRandomThemes(); });
}
...
function generateRandomThemes() {
    ...
    contentDiv.querySelectorAll('.srt-theme-item').forEach(item => {
        item.addEventListener('click', (e) => { insertTextToChat(e.target.getAttribute('data-theme')); });
    });
}
